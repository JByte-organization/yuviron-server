...
def _render_acl_block(acl_nets: tuple[str, ...]) -> str:
    ...
    return (
        "    # Allow recursive DNS queries only from configured private networks.\n"
        "    # Everything else is blocked before forward can answer.\n"
        "    acl {\n"
        f"{allow_lines}\n"
        "        block\n"
        "    }\n"
...
def render_corefile(
    *,
    ...
    return (
        ...
        "#\n"
        "# =========================================================\n"
        f"# Yuviron private {environment} domains via CoreDNS\n"
        "# =========================================================\n"
        "#\n"
        f"# All {domain} names resolve to the target IP below.\n"
        "# Use a private Tailnet/VPN/LAN address here, not a public Internet IP.\n"
        f"# Fallback recursive DNS is restricted by acl to: {', '.join(acl_nets)}\n"
        "#\n"
        "# Examples generated from current routes:\n"
        f"{example_lines}\n"
        "#\n"
        "# =========================================================\n\n"
        f"{domain} {{\n"
        f"    # Resolve {domain} and all subdomains to the private target IP.\n"
        "    template IN A {\n"
        f"        match (^|\\.){domain_pattern}\\.$\n"
        f"        answer \"{{{{ .Name }}}} 60 IN A {ip_address}\"\n"
        "    }\n\n"
        "    # Explicitly deny IPv6 AAAA queries to avoid unresolved fallthrough.\n"
        "    template IN AAAA {\n"
        f"        match (^|\\.){domain_pattern}\\.$\n"
        "        rcode NXDOMAIN\n"
        "    }\n\n"
        ...
        "# =========================================================\n"
        "#\n"
        "# This block is also protected by acl, so CoreDNS cannot become\n"
        "# an open recursive resolver for clients outside the private network.\n"
        "#\n"
        ". {\n"
        f"{acl_block}\n"
        "    # Resolve all other allowed queries through public DNS.\n"
