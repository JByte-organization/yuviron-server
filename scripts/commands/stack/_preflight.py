# =============================================================================
# scripts/commands/stack/_preflight.py — Preflight-проверки перед запуском.
#
# cmd_preflight() выполняет все проверки необходимые для безопасного запуска:
#   1. Доступность инструментов (docker, mkcert или certbot и др.)
#   2. Доступность Docker daemon
#   3. Интернет-соединение и DNS (если не --skip-connectivity-check)
#   4. Наличие и свежесть generated/<env>/ (через manifest.env хэши)
#   5. Наличие всех обязательных переменных окружения
#   6. Валидность env-политики (нет слабых паролей, нет dev-настроек в prod)
#   7. Существование TLS-сертификатов, htpasswd, routes.env
#   8. Наличие и права директории storage/
#   9. Достаточно места на диске
#   10. Существование shared Docker network
#   11. Маршруты из routes.env валидны
#   12. "docker compose config" успешно раскрывает все переменные
#   13. "nginx -t" успешно валидирует сгенерированный nginx.conf
#   14. Права на storage для backend-процесса
#
# Режим --isolated: запускает проверки в отдельном compose-проекте
# на временных портах (18080/18443) чтобы не мешать работающему окружению.
#
# PreflightContext — датакласс, содержащий все пути и параметры.
# Передаётся между функциями preflight_checks как единый объект.
# =============================================================================
