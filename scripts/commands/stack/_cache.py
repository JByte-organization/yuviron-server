...
#   (без --path)          — очистить весь CDN-кэш
#
# Алгоритм для конкретного файла:
#   1. Строим cache key = "https" + host + path
#   2. MD5(cache_key) — так nginx именует файлы кэша
#   3. Ищем файл с таким именем в NGINX_MEDIA_CACHE_DIR и удаляем
#
# Это "ручная инвалидация кэша" — используется когда медиа-файл обновился,
# а кэш ещё не истёк (nginx кэширует по Cache-Control заголовкам бэкенда).
# =============================================================================
