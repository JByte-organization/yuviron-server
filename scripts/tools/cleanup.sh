#!/bin/bash
# =============================================================================
# scripts/tools/cleanup.sh — Комплексная очистка диска на сервере.
#
# Выполняет следующие шаги (14 пунктов):
#   1. Кэш Docker builder
#   2. Dangling образы (без тегов)
#   3. Все unused образы
#   4. Остановленные контейнеры
#   5. Неиспользуемые Docker сети
#   6. Логи Docker-контейнеров (обрезает до нуля)
#   7. Системные журналы systemd (оставляет последние 100MB)
#   8. Кэш apt-get
#   9. Временные файлы /tmp и /var/tmp
#  10. Пустые .log файлы старше 7 дней в logs/
#  11. Python __pycache__ директории
#  12. Папка generated/ (спрашивает — удаление требует re-init)
#  13. Папка certs/ (спрашивает — удаление требует regeneration сертификатов)
#  14. Папка .tmp/ (спрашивает — очищает временные кэши)
#
# Флаги:
#   -y   автоматически отвечать "да" на все подтверждения
#
# Запуск: ./scripts/tools/cleanup.sh
# Запуск без подтверждений: ./scripts/tools/cleanup.sh -y
# =============================================================================
set -e
...
confirm() {
  # При -y всегда возвращаем 0 (да), иначе — спрашиваем у пользователя.
  # Enter без ввода считается как "Y" (да по умолчанию).
...
# Запрашиваем пароль sudo заранее чтобы потом не прерывать процесс очистки
sudo -v

# Фоновый процесс обновляет sudo timestamp каждые 60 секунд.
# Это нужно чтобы sudo не истекало во время длинных операций.
while true; do
  sudo -n true
  sleep 60
  kill -0 "$$" || exit
