...
Management routes (`seq`, `aspire` и т.п.) закрываются двумя слоями: HTTP Basic Auth и IP/CIDR allowlist. `NGINX_ADMIN_ALLOWLIST` по умолчанию собирается как `127.0.0.1/32,${NGINX_PRIVATE_ACCESS_CIDRS}`; `NGINX_PRIVATE_ACCESS_CIDRS` не должен быть всем Tailscale CGNAT `100.64.0.0/10`, потому что это общий диапазон Tailscale. В tracked default используется узкий пример `100.81.228.0/24`; для реального окружения переопредели значение в `env/<env>.env` на конкретные Tailnet device `/32`, subnet-route CIDR или другой VPN, например `NGINX_PRIVATE_ACCESS_CIDRS=100.81.228.68/32,10.8.0.0/24`.

Media CDN route задаётся в `config/routes.yml` как route `i` с `host_strategy: subdomain` и `media_proxy: true`. Для dev это генерирует host `dev-i.<domain>`, для prod — `i.<domain>`. Внешний URL имеет вид `https://dev-i.yuviron.com/<hash>` без папок и расширений, а nginx переписывает его во внутренний backend endpoint `/i/<hash>`.
Для этого route nginx включает `proxy_cache media_cache`: успешные `200` ответы кэшируются на `365d`, `404` — только на `1m`, CORS открыт через `Access-Control-Allow-Origin: *`, а `OPTIONS` завершается на edge с `204`. Browser cache header `Cache-Control: public, immutable, max-age=31536000` выставляется только для успешных ответов, чтобы отсутствующие картинки не застревали в браузерах на год. Upstream cache headers и cookies скрываются/игнорируются на edge, потому что контракт media CDN считается immutable по hash.
...
Aspire Dashboard собирается из `infra/docker/aspire-dashboard/Dockerfile` на базе официального `mcr.microsoft.com/dotnet/aspire-dashboard:9.0`. У standalone dashboard 9.0 нет пригодного `/health` endpoint: GET `/health` уводит на login, а HEAD `/health` возвращает `404`. Поэтому derived image добавляет один BusyBox binary, а Docker healthcheck в exec-form делает TCP probe `127.0.0.1:18888` через `nc -z -w 3`. Это проверяет listener самого dashboard, а не просто наличие установленного .NET runtime.
Сам nginx также имеет Docker healthcheck: контейнер локально проверяет `http://127.0.0.1/health` и срок действия default-сертификата `/etc/nginx/certs/<env>-<domain>.pem`. TLS handshake через `openssl s_client` намеренно не используется внутри Docker healthcheck, чтобы startup health не зависел от более хрупкого HTTPS probe. Отдельная HTTPS-проверка остаётся в `stack smoke`: она проходит по каждому route host через `curl --resolve ... 127.0.0.1`.
`/health` объявлен в default HTTP/HTTPS server-блоках до `return 444`, поэтому HTTP healthcheck не зависит от внешнего `Host`, а smoke дополнительно ловит проблемы TLS listener и route certificate binding.
...
`scripts/core/env_validation.py` валидирует итоговый `generated/<env>/deploy.env` через `jsonschema` по `env/schema.json`; локальный код отвечает только за custom `format` checks (`port`, `cidr-list`, `nginx-rate`, `docker-memory` и т.п.) и project-specific policy. Пропущенные обязательные ключи и неверные форматы считаются ошибками, а неизвестные ключи выводятся как warning, чтобы новая переменная не появилась бесшумно. Тесты дополнительно проверяют, что ключи из `env/common.env`, `env/example.env` и `${...}`-переменные из `infra/compose.yml` объявлены в схеме.
...
`migrator` вынесен в compose profile `migrate` и не является init-зависимостью backend/media-worker. `./scripts/cli.py stack up <env>` запускает его явно перед основным `up`, а `./scripts/cli.py stack migrate <env>` позволяет выполнить тот же шаг вручную. Внутри `infra/docker/dotnet/migrator.sh` есть production guard: при `ASPNETCORE_ENVIRONMENT=Production` контейнер падает, пока явно не передан `ALLOW_PRODUCTION_MIGRATE=true`. После guard сначала выполняется `dotnet ef database update`, затем запускается `dotnet ef migrations list`; команда падает, если после update остаются pending migrations.
